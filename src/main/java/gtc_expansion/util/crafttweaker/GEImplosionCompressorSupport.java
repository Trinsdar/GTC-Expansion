...
@ZenRegister
public class GEImplosionCompressorSupport {
    @ZenMethod
    ...
    private static final class ImplosionCompressorRecipeAction implements IAction {
        ...
        public void apply() {
            ...
            GETileMultiImplosionCompressor.addRecipe(input1, tnt, totalEu, output);
        }

        @Override
        public String describe() {
            return String.format(Locale.ENGLISH, "Add Recipe[%s, %s, %s -> %s] to %s", input1, tnt, totalEu, Arrays.deepToString(output), GERecipeLists.IMPLOSION_COMPRESSOR_RECIPE_LIST);
        }
    }
}
