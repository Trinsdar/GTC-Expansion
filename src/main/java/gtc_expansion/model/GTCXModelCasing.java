...
public class GTCXModelCasing extends BaseModel {
    ...
    public void init() {
        this.quads = this.createDoubleList(7, 64);
        ...
        for (int i = 0; i < 2; ++i) {
            for (int j = 0; j < 64; j++){
                ...
                for (EnumFacing facing : EnumFacing.values()) {
                    ...
                    if (sprite != null) {
                        this.quads[facing.getIndex()][j].add(this.getBakery().makeBakedQuad(this.getMinBox(facing, box), this.getMaxBox(facing, box), face, sprite, facing, sideRotation, (BlockPartRotation) null, false, true));
                    }
                }
            }
...
//            return array[index];
    ...
    public List<BakedQuad> getQuads(@Nullable IBlockState state, @Nullable EnumFacing side, long rand) {
        ...
        if (!(state instanceof BlockStateContainerIC2.IC2BlockState)) {
            ...
            i = ((BlockStateContainerIC2.IC2BlockState) state).getData();
        }
        return this.quads[side == null ? 6 : side.getIndex()][i];
    }
